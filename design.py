...
def compute_masses(avl_fname, cylinders_fname, proptype):
    ...
    for row in cylinders:
        ...
        if cyltype == "cabin":
            empty_mass = 0
            landing_mass = takeoff_mass = row[2] * (row[4] - row[3]) / total_cabin_area * PAYLOAD
        else:
    ...
    return (empty_mass, landing_mass, takeoff_mass, eta)
...
quit
        ...
        with open("x", "w") as f:
            f.write(X)
        os.system(f"{XFOIL} < x > /dev/null")
        try:
            with open("save.txt") as f:
                cdv = float(f.read().split("\n")[-2].strip().split()[2])
            os.remove("save.txt")
            os.remove("dump.txt")
            os.remove("x")
        except:
...
def compute_aoa_and_induced_drag(avl_fname, cl):
...
quit
    ...
    with open("a", "w") as f:
        f.write(A)
    os.system(f"{AVL} < a > /dev/null")
    try:
        with open("forces.txt") as f:
...
if __name__ == '__main__':
    ...
    print("AIRCRAFT MASS")
    me, ml, mto, eta = compute_masses(args.avl_fname, args.cylinders_fname, proptype)
    ...
    aoa, cdi = compute_aoa_and_induced_drag(args.avl_fname, cl)
